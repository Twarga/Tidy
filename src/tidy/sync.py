...
__all__ = ["pull_all", "pull_repo", "sync_all", "sync_repo"]
...
def pull_repo(cfg: dict, repo: dict) -> dict:
    """Pull remote changes for one repo (fetch + pull --rebase, no commit/push)."""
    git.ensure_git()
    started = monotonic()
    repo_id = repo["id"]
    path = repo["path"]
    ...
    if not git.has_remote(path):
        ...
        log_warn(message, repo_id)
        update_stats(cfg, last_run=_now_iso(), last_error=message)
        return {"ok": False, "id": repo_id, "message": message, "elapsed": _elapsed(started)}
    ...
    elapsed = _elapsed(started)
    message = f"{repo_id}: pulled ✓ ({elapsed}s)"
    log_info(message, repo_id)
    update_stats(cfg, last_run=_now_iso())
    return {"ok": True, "id": repo_id, "message": message, "elapsed": elapsed}


def pull_all(cfg: dict) -> dict:
    """Pull remote changes for every configured repo."""
    results = [pull_repo(cfg, repo) for repo in cfg["repos"]]
